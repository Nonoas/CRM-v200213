...
public class VipInfoDao extends MyDao<VipBean> {
	...
	private static final String INSERT_INFO = "insert into "
			+ "user_info(id,name,acronym,sex,birthday,card_level,balance,frequency,cumulative,"
			+ "address,integral,telephone,idcard,career,email,other,admission_date,photo) "
			+ "values(#{id},#{name},#{acronym},#{sex},#{birthday},#{card_level},#{balance},"
	...
	private static final String UPDATE_INFO = "update user_info set "
			+ "name=#{name},acronym=#{acronym},sex=#{sex},birthday=#{birthday},card_level=#{card_level},"
	 ...
	 * 本类的单例对象
	 */
	private static final VipInfoDao INSTENCE=new VipInfoDao();
	
	/**
	 * 获取本类的单例对象
	 * @return VipInfoDao的单例对象
	 */
	public static VipInfoDao getInstence() {
		return INSTENCE;
	}
